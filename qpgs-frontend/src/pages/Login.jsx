...
export default function Login({ onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onLoginSuccess) {
        onLoginSuccess(token, role, username, refreshToken);
      }
      navigate("/dashboard");
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex" style={{ backgroundColor: "#FAF8F3" }}>
      ...
      >
        ...
        <div className="relative z-10 animate-fade-in-up" style={{ animationDelay: "0.2s", opacity: 0 }}>
          <div
            className="inline-flex items-center justify-center px-4 h-12 rounded-xl mb-10 shadow-lg"
            style={{ background: "linear-gradient(135deg, #B8863B 0%, #966A1F 100%)" }}
          >
            <span className="qpgs-serif text-lg font-bold tracking-wider text-white">Q-Gen</span>
          </div>
          <h1 className="qpgs-serif text-white text-4xl leading-tight max-w-sm">
            Set your question papers in order.
          </h1>
          <p className="qpgs-sans text-sm mt-5 max-w-xs leading-relaxed" style={{ color: "#AEB8CC" }}>
            Manage your question bank and generate randomized papers, ready to export in minutes.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm animate-fade-in-up" style={{ animationDelay: "0.1s", opacity: 0 }}>
          <div className="mb-8 md:hidden">
            <div
              className="inline-flex items-center justify-center px-4 h-10 rounded-xl mb-4 shadow-md"
              style={{ background: "linear-gradient(135deg, #B8863B 0%, #966A1F 100%)" }}
            >
              <span className="qpgs-serif text-sm font-bold tracking-wider text-white">Q-Gen</span>
            </div>
          </div>
